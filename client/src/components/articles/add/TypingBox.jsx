...
const TypingBox = ({title , setTitle,content , setContent ,editedTitle,EditedContent, editing, handleChangeEditedArticle}) => {
			...
			<div className="lg:[width:100%] [height:600px]" >
				<div className="flex flex-col [width:100%] mb-2">
					...
					<input className="pl-2 py-1 border-t-gray-900 focus:text-gray-900 text-gray-600 [width:100%] text-[24px] [border:1px_solid_#ddd] [outline:none] [border-radius:7px]"
						...
						value={editing ? editedTitle :title}
		                onChange={editing ? handleChangeEditedArticle : (e)=>setTitle(e.target.value)}
					/>
				</div>
				<span className="xs:text-[26px] sm:text-[30px] lg:text-[36px] text-[#666]">content</span>
				<textarea placeholder="text" wrap="off" id="content"
	                ...
	                onChange={editing ? handleChangeEditedArticle :(e)=>setContent(e.target.value)}
				>
				</textarea>
			</div>	
